<div @if(isset($name)) onclick="downloadVarData('{{ $name }}','{{ $sensor_name ?? 'null'}}',@if($name == 'ee-e-activa' || $name == 'ee-e-reactiva' || $name == 'ee-e-aparente') 'consumption-date' @else 'date-filter' @endif {{')'}}" @endif class=" cursor-pointer p-2 {{ $bg ?? 'bg-primary-300' }} {{ (isset($extraClasses))?implode(' ',$extraClasses):'' }} rounded overflow-hidden position-relative text-white @if(isset($mb)) {{ $mb }} @else mb-g @endif @if($value > 99999999 && $value < 1000000000) pb-1 pt-2 @endif @if($value > 999999999) pb-1 pt-3 @endif">
    <div class="">
