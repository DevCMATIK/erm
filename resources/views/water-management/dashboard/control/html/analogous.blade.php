@if($analogous_sensor->no_chart_needed != 1)
    <a href="/getDataAsChart/{{ $analogous_sensor->sensor->device_id }}/{{ $analogous_sensor->sensor->id }}"  {!! makeLinkRemote() !!} class="d-block  rounded-plus mb-1 px-2 py-1 text-center" id="d_{{$analogous_sensor->id}}_{{$analogous_sensor->sensor->id}}">
@else
    <div class="d-block  rounded-plus mb-1 px-2 py-1 text-center" id="d_{{$analogous_sensor->id}}_{{$analogous_sensor->sensor->id}}">
@endif
