        ...
        <div class="row">
            <div class="col">
                <div class="dd border p-2" id="nestable">
                    <ol class="dd-list " id="list-1">
                        @if(optional($checkPoint->grids)->first())
                            @foreach($checkPoint->grids()->where('column',1)->get() as $grid)
                                @if($grid->sensor_id === null)
                                    <li class="dd-item border p-1" data-id="blank_{{ $blank_count }}">
                                    ...
                                    @endphp
                                @else
                                    <li class="dd-item" data-id="{{ $grid->sensor->id }}">
                                        <div class="dd-handle">{{ $grid->sensor->name }}</div>
                                    </li>
                                @endif
                        ...
                        @else
                            @foreach($checkPoint->devices as $device)
                                @foreach($device->analogous_sensors as $sensor)
                                    <li class="dd-item" data-id="{{ $sensor->id }}">
                                        <div class="dd-handle">{{ $sensor->name }}</div>
                                    </li>
                                @endforeach
                            @endforeach
                        @endif
            ...
            <div class="col">
                <div class="dd border p-2" id="nestable2">
                    <ol class="dd-list @if($checkPoint->grids()->where('column',2)->count() === 0) dd-empty @endif " id="list-2">
                        @if(optional($checkPoint->grids)->first())
                            @foreach($checkPoint->grids()->where('column',2)->get() as $grid)
                                @if($grid->sensor_id === null)
                                    <li class="dd-item border p-1" data-id="blank_{{ $blank_count }}">
                                        <div class="dd-handle"></div>
                                    </li>
            ...
            <div class="col">
                <div class="dd border p-2" id="nestable3" >
                    <ol class="dd-list @if($checkPoint->grids()->where('column',3)->count() === 0) dd-empty @endif" id="list-3">
                        @if(optional($checkPoint->grids)->first())
                            @foreach($checkPoint->grids()->where('column',3)->get() as $grid)
                                @if($grid->sensor_id === null)
                                    <li class="dd-item border p-1" data-id="blank_{{ $blank_count }}">
                                        <div class="dd-handle"></div>
                                    </li>
...
<form class="my-5" role="form"  id="menu-serialization-form">
    @csrf
    <input type="hidden" id="blank_count" value="{{ $blank_count }}">
    <div class="form-group">
        ...
        <a href="javascript:void(0);"  onClick="resetGrid({{ $checkPoint->id }})" class="btn btn-danger reset-grid">Reset</a>
    </div>
    <div class="form-group">
...
<div class="row">
    ...
    <div class="col-6">
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="work_code" value="{{ $checkPoint->work_code }}">
        </div>
    </div>
</div>
...
<script>
   function saveDga()
   {
       ...
       $.get('/check-point/dga/{{ $checkPoint->id }}?work_code='+work_code+'&dga_report='+dga_report,function(data){
...
{!!  makeValidation('#menu-serialization-form','/CheckPointSensorsSerialization/'.$checkPoint->id, "showGrid(".$checkPoint->id.")") !!}
