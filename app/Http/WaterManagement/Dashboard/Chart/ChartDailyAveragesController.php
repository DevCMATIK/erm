...
class ChartDailyAveragesController extends Controller
{
    public function __invoke(Request $request,$device_id,$sensor_id)
    {
        ...
        if ($request->has('date') && $request->date != '') {
                $query = SensorDailyAverage::with('sensor')
    ...
    protected function transformData($rows,$var){
        ...
        foreach ($rows as $key => $row) {
            array_push($array, [
                ...
                'name' => Carbon::parse($row->date)->toDateString()
            ]);
        }
